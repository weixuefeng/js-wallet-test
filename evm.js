...
async function signTransactionTest()  {
  var wallet = ethers.HDNodeWallet.fromMnemonic(ethers.Mnemonic.fromPhrase(mnemonic));

  var tx = new ethers.Transaction();
  tx.to = "0x6BD5a1F893A7e8891f79dd0FA6CE6d286dF410c1";
  tx.gasPrice = 159524730353;
  tx.gasLimit = 21000;
  tx.value = 1;
  tx.nonce = 5;
  tx.chainId = 137;
  console.log(tx.unsignedSerialized);
  var res = await wallet.signTransaction(tx)
  console.log(res);
}


async function main() {
  await signTransactionTest()
}
